...
#' @export
makeOxygenLight <- function(obj, add_default = TRUE, add_fields = "export", 
                             use_dictionary = NULL, print = TRUE, ...) {
  header_add <- sinew_opts$get()
  lbl <- deparse(substitute(obj))
  lbl <- gsub("\"", "", lbl)
  if (is.character(obj)) 
  ...
  if (inherits(obj, c("data.frame", "tibble"))) {
    ...
    header <- c(title = "#' @title DATASET_TITLE", 
                description = "#' @description DATASET_DESCRIPTION", 
                format = sprintf("#' @format A data frame with %s rows and %s variables:", 
                                 nrow(obj), length(cl)))
    add_fields <- setdiff(add_fields, c("export", "examples", 
                                        "seealso", "rdname"))
    ret <- sprintf("%s\n%s\n%s%s", paste(header, collapse = "\n"), 
                   sprintf("#' \\describe{\n%s \n#'}", items), 
                   ifelse(!is.null(add_fields), {
                     paste(sprintf("#' @%s %s", names(header_add[add_fields]), 
                                   header_add[add_fields]), collapse = "\n")
                   }, ""), sprintf("\n\"%s\"", lbl))
  }
  if (inherits(obj, c("function"))) {
    importList <- list(...)
    ...
    if (import == "list()") 
      import <- ""
    cutOFF <- switch("cut" %in% names(importList), 
                     importList$cut, 3)
    if (import == "") 
      add_fields <- add_fields[!grepl("seealso", 
                                      add_fields)]
    if ("seealso" %in% add_fields) 
      header_add <- c(header_add, seealso = paste0(makeSeeAlso(obj, 
    ...
    params <- sprintf("#' @param %s %s", names(out), names(out))
    header <- c(title = sprintf("#' @title %s", strsplit(lbl," ")[[c(1,1)]]), 
                description = "#'")
    footer <- c(return = "#'")
    ret <- sprintf("%s\n%s\n%s\n%s", 
                   paste(header, collapse = "\n"), 
